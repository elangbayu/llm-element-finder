...
public class BasePage {
    ...
    private static final Path ELEMENT_SELECTORS_FILE = Path.of("src/test/resources/elementSelectors.properties");
    ...
    /**
     * A reusable click method that:
     * 1) Tries to get a cached selector by 'key' from a .properties file.
     * 2) Falls back to 'defaultSelector' if cache fails.
     * 3) Falls back to AI-based search if both fail.
     * 4) Saves newly found selector from AI to the .properties file.
     *
     * @param key             The key used in the .properties file for caching the
     *                        selector.
     * @param defaultSelector The conventional/default selector to try if caching
     *                        fails.
     * @param aiPrompt        The prompt to send to an AI-based mechanism if both
     *                        caching
     *                        and default fails.
    ...
    public static void click(String key, By defaultSelector, String aiPrompt) {
        ...
        try {
            ...
            WebElement element = wait.until(ExpectedConditions.visibilityOfElementLocated(defaultSelector));
            element.click();
     ...
     * that is presumably a `By.cssSelector("someCss")`.
     *
