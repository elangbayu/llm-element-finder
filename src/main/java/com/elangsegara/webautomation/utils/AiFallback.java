...
public class AiFallback {
    public static By getElementFromAi(String queryElement) {
        ...
        OpenAIClient client = OpenAIOkHttpClient.builder().baseUrl("https://openrouter.ai/api/v1")
                .apiKey(System.getenv("OPENROUTER_API_KEY")).build();
        ResponseFormatJsonSchema.JsonSchema.Schema schem = ResponseFormatJsonSchema.JsonSchema.Schema.builder()
                .putAllAdditionalProperties(
                        Map.of(
                                "type", JsonValue.from("object"),
                                "properties", JsonValue.from(Map.of(
                                        "css_selector", JsonValue.from(Map.of(
                                                "type", "string",
                                                "description", "CSS Selector of the given element output.")))),
                                "required", JsonValue.from(List.of("css_selector"))))
                .build();
        ResponseFormatJsonSchema.JsonSchema schemas = ResponseFormatJsonSchema.JsonSchema.builder()
                .name("CSS Selector Extractor").strict(true).schema(schem).build();
        ...
        ChatCompletionCreateParams params = ChatCompletionCreateParams.builder()
                .addSystemMessage(
                        "Given the HTML source, extract the element with the given query. Always return in json format.")
        ...
        return By.cssSelector(client.chat().completions().create(params).choices().stream()
    ...
    private static String domProcessor(WebDriver driver) {
        ...
        return (String) js.executeScript(
                "function isElementInViewport(el) {"
                        ...
                        + "  );"
                        + "}"
                        + "function analyzeInteractableNodes(root) {"
                        + "  var result = { html: '', interactableCount: 0 };"
                        + "  if (!root) return result;"
                        + ""
                        + "  var clone = root.cloneNode(true);"
                        + "  if (isElementInteractable(root)) {"
                        + "    result.html = clone.outerHTML;"
                        + "    result.interactableCount++;"
                        + ""
                        + "    Array.from(root.children).forEach(function(child) {"
                        + "      var childAnalysis = analyzeInteractableNodes(child);"
                        + "      result.interactableCount += childAnalysis.interactableCount;"
                        + "    });"
                        + "  }"
                        + "  return result;"
                        + "}"
                        + ""
                        + "var analysis = analyzeInteractableNodes(arguments[0]);"
                        + "return JSON.stringify({"
                        + "  html: analysis.html,"
                        + "  interactableCount: analysis.interactableCount,"
                        + "  inViewport: isElementInViewport(arguments[0])"
                        + "});",
                nextDiv);
    }
